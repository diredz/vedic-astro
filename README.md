# 🪐 Jyotish — Local Vedic & Western Astrology Engine

Full offline Vedic and Western Tropical astrology engine on your MacBook Air M4.
Features a rich CLI and a beautiful Web GUI (`gui.py`).
...
### 🎨 Web GUI (Recommended)
Run the Flask server to open the visually rich web interface:
```bash
python gui.py
```
...
```text
vedic-astro/
├── gui.py               # Flask Web GUI (HTML/CSS/JS frontend)
...
│   ├── western.py       # Western Tropical astrology engine
...
└── llm/
    ├── interpreter.py          # Ollama integration (Vedic)
    └── western_interpreter.py  # Ollama integration (Western & Compare)
```
